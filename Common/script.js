...
function renderDate() {
  ...
  document.getElementById(
    "toDay"
  ).innerHTML = `${day}&nbsp${month}&nbsp${year}`;
}
...
function setTimeToAlarmField() {
  let now = new Date();
  let timeForm = document.querySelector(
    "#setAlarm .setAlarmFormContainer .setTimeForm"
  );
  const timeFormChild = timeForm.children;
  if (timeFormChild) {
    timeFormChild.hour.value = (now.getHours() % 12)
      .toString()
      .padStart(2, "0");
    (timeFormChild.second.value = now.getSeconds().toString().padStart(2, "0")),
      (timeFormChild.minute.value = now
        .getMinutes()
        .toString()
        .padStart(2, "0")),
...
function getDayOfWeek(timeZone) {
  ...
  let dayOfWeekStr = formattedDayParts.find(
...
function checkForAlarm(key) {
  ...
  if (
    oAlaram != null &&
    typeof oAlaram === "object" &&
    Object.keys(oAlaram).length > 0
  ) {
    let { dayOfWeek, expires } = JSON.parse(storedData);
    if (
      oAlaram &&
      oAlaram.status &&
      oAlaram.days.has(JSON.stringify(dayOfWeek))
    ) {
...
function getTimeZone() {
  let timeZone = localStorage.getItem("timeZone");
  if (!timeZone) setTimeZone("Asia/Kolkata");
...
function createAlarm(oEvent) {
  ...
  const timeFormChild = timeForm.children;
  if (
    timeFormChild.hour.value >= 1 &&
    timeFormChild.hour.value <= 12 &&
    timeFormChild.minute.value >= 0 &&
    timeFormChild.minute.value <= 59 &&
    timeFormChild.second.value >= 0 &&
    timeFormChild.second.value <= 59 &&
    (timeFormChild.format.value == "AM" || timeFormChild.format.value == "PM")
  ) {
    const weeks = document.querySelector(
      "#setAlarm .setAlarmFormContainer .weeks-container"
    );
    let payload = {
...
function setAlarmDetailsToDB(alarmInfo) {
  ...
  let key =
    alarmInfo.hour + alarmInfo.minute + alarmInfo.second + alarmInfo.format;
...
class Alarm {
  ...
  constructor(alaramInfo) {
    ...
    if (this.#days.size == 0) this.#status = false;
  ...
  updateWeekDay(day) {
    ...
    if (this.#days.has(day)) this.#days.delete(day);
    ...
    setAlarmDetailsToDB(payLoad);
    if (renderAction) this.renderActionButton();
  }
...
function getCurrentTimeInMinutes() {
  ...
  return (
    parseInt(now.hour) * 3600 + parseInt(now.minute) * 60 + parseInt(now.second)
  );
}
...
function sortObjectByCloseness(obj) {
  const sortedEntries = Object.entries(obj).sort((a, b) => {
    const closenessA = calculateCloseness(
      a[1].hour,
      a[1].minute,
      a[1].second,
      a[1].format,
    ...
    const closenessB = calculateCloseness(
      b[1].hour,
      b[1].minute,
      b[1].second,
      b[1].format,
